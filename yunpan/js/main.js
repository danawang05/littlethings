...
wy.reName.addEventListener('click', function(e) {
    ...
    setFileItemName(checkedBuffer, true); //重命名成功
});
...
// 删除功能
wy.rmFile.addEventListener('click', function(e) { //点击按钮
    const { checkedBuffer } = wy; //设置缓存
...
function deletFiles(checkedBuffer) { //删除功能
    const checkedEles = getCheckedFileFromBuffer(checkedBuffer);
    const { fsContainer } = wy;

    checkedEles.forEach(function(item, i) { //查看每一个
        const { fileId, fileNode } = item;

        fsContainer.removeChild(fileNode.parentNode); //从父节点删除
...
// -------------------------------------------------------------------
// 新建文件夹
wy.newDir.addEventListener('click', function(e) { //监听点击事件
    initCheckedFiles(); //初始化文件夹

    const { currentListId, fsContainer, checkedBuffer } = wy; //设置常量为局部变量

    const newFolderData = { //建立新文件数据模型
        id: Date.now(),
        name: '',
        pId: wy.currentListId
    };

    const newFolderNode = createFileNode(newFolderData); //创建文件

    const fileWrap = newFolderNode.querySelector('.file-wrap'); //获取文件

    fsContainer.insertBefore(newFolderNode, fsContainer.firstElementChild);
    //设置将新建立的文件插入第一位置

    checkNodeData(fileWrap); //查询节点数据

    showEmptyInfo(); //是否显示目录为空

    setFileItemName( //命名文件
        checkedBuffer,
        false,
        (name) => { //新建立成功
